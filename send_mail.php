...
<div class="mail-box">
    ...
    WHERE (l.sender =" . $_SESSION['user_id'] . " and l.receiver!=" . $_SESSION['user_id'] . ") and l.document_status=1 GROUP BY sol";
    $result_sol = $conn->query($sql_sol);
    while ($row_sol = mysqli_fetch_assoc($result_sol)) {
        echo "<a href='?year=" . $row_sol['sol'] . "'>" . $row_sol['sol'] . "</a>&nbsp";
    }
    if (isset($_GET['year'])) {
        $sol = $_GET['year'];
    ...
    INNER JOIN documents d ON l.document_id=d.document_id WHERE l.sender=" . $_SESSION['user_id'] . " AND l.is_group=0  " . $where . " and l.receiver !=" . $_SESSION['user_id'] . "  and l.document_status=1 GROUP BY l.unical
    ...
    WHERE l.is_group=1 AND l.sender =" . $_SESSION['user_id'] . "  " . $where . " and l.receiver !=" . $_SESSION['user_id'] . " and l.document_status=1 GROUP BY l.unical ORDER BY input_date desc 
");
