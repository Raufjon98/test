...
<!--            --><?//=uniqid()?>
    ...
    <div class="table-responsive">
        ...
        if (isset($_GET['god'])) {
            ...
            INNER JOIN documents d ON l.document_id=d.document_id WHERE l.sender=" . $_SESSION['user_id'] . " AND l.is_group=0  " . $where . " AND (l.receiver != 0 or l.receiver !=" . $_SESSION['user_id'] . ") and l.document_status=1 GROUP BY l.unical
            ...
            INNER JOIN documents d ON l.document_id=d.document_id WHERE  (l.receiver=" . $_SESSION['user_id'] . "  or l.receiver=0)  " . $where . " and l.sender !=" . $_SESSION['user_id'] . " and l.document_status=1 GROUP BY l.unical
            ...
            WHERE l.is_group=1 AND l.sender =" . $_SESSION['user_id'] . "  " . $where . " and (l.receiver != 0 or l.receiver !=" . $_SESSION['user_id'] . ") and l.document_status=1 GROUP BY l.unical
        ...
        while ($row = mysqli_fetch_assoc($nat)) {
            echo "<tr >";
            echo "<td> <input id='".$row['unical']."' type='checkbox' class='i-checks delete_mail' name='mail_check' style='margin: auto'></td>";
            ...
            if ($row['is_file'] == 1) {
                ...
                if (strlen($row['document_name'] . "</a> </b><comment> - </comment>" . (strip_tags($row['comments']))) > 75) {
                    echo "<td width='40%' ><div class='col-md-10'><b><a href='index.php?id_document=" . $row['unical'] . "'>" . mb_substr($row['document_name'] . "</a> </b><comment> - </comment>" . (strip_tags($row['comments'])), 0, 75) . "<comment>...</comment></div>&nbsp<div class='col-md-1' align='right' style='margin-left: 5%'><h3><i class='fa fa-paperclip'></i></h3></div> </td>";
                } else {
                    echo "<td width='40%'><div class='col-md-10'><b><a href='index.php?id_document=" . $row['unical'] . "'>" . $row['document_name'] . "</a> </b><comment> - </comment>" . strip_tags($row['comments']) . "</div><div class='col-md-1' align='right' style='margin-left: 5%'><h3><i class='fa fa-paperclip'></i ></h3> </div></td>";
                }
            } else {
                if (strlen($row['document_name'] . "</a> </b><comment> - </comment>" . (strip_tags($row['comments']))) > 75) {
