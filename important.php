...
<div class="mail-box" style="margin: 3px">
    ...
    <div class="table-responsive">
        ...
        $sql_sol = "SELECT DATE_FORMAT(input_date, '%Y') sol  FROM   documents d
                ...
                WHERE (l.sender=" . $_SESSION['user_id'] . " or (l.receiver=" . $_SESSION['user_id'] . " or receiver=0 or l.receiver=gm.group_id )) and l.document_status=2 GROUP BY sol";
        $result_sol = $conn->query($sql_sol);
        while ($row_sol = mysqli_fetch_assoc($result_sol)) {
            echo "<a href='?important_year=" . $row_sol['sol'] . "'>" . $row_sol['sol'] . "</a>&nbsp";
        }
        $where_me_on_group = " or (gm.group_id=l.receiver AND gm.user_id=" . $_SESSION['user_id'] . ")";
        if (isset($_GET['important_year'])) {
            $sol = $_GET['important_year'];
        ...
        $nat = $conn->query ("SELECT l.unical, d.date_order, d.document_name, d.extension, d.comments, l.sender, DATE_FORMAT (d.input_date, '%d.%m.%Y %H:%i:%s') AS input_date, l.receiver, l.is_file, l.is_group, d.document_id, d.document_path, d.file_name, 0 is_ready FROM letter l
            INNER JOIN documents d ON l.document_id=d.document_id WHERE l.sender=" . $_SESSION['user_id'] . " AND l.is_group=0  " . $where . " AND  (l.receiver != 0 or l.receiver !=" . $_SESSION['user_id'] . ") and l.document_status=2 GROUP BY l.unical
            ...
            INNER JOIN documents d ON l.document_id=d.document_id WHERE  (l.receiver=" . $_SESSION['user_id'] . "  or l.receiver=0)  " . $where . " and l.sender !=" . $_SESSION['user_id'] . " and l.document_status=2 GROUP BY l.unical
            ...
            WHERE l.is_group=1 AND l.sender =" . $_SESSION['user_id'] . "  " . $where . " and (l.receiver != 0 or l.receiver !=" . $_SESSION['user_id'] . ") and l.document_status=2 GROUP BY l.unical
            ...
            WHERE l.is_group=1 AND l.receiver =" . $_SESSION['user_id'] . " or l.receiver=0  " . $where_me_on_group . " " . $where . " and l.sender!=" . $_SESSION['user_id'] . " and l.document_status=2 GROUP BY l.unical ORDER BY input_date desc ");
